...
rms(n-3)=sqrt(dot(transpose(res_all),res_all)/length(res_all));%root means square of dot product / matrix length
end

n_mat=4:300;
plot(log10(n_mat),log10(rms),'x')

[log_b,log_bint,log_r]=regress(transpose(log10(rms)),transpose(log10(n_mat)));
